...
10. Review: use `codex-simplicity-review` for overengineering risk and `codex-review-panel` for meaningful diffs, plans, docs, or high-risk delivery; record accepted and rejected findings. If accepted findings require project-file edits, run `workflow-state transition review-changes-requested`, use `receiving-code-review`, implement the scoped fix, then return through `execution-complete`, verification, and review. Use `review-complete` or `review-skipped` only when no project fix remains.
...
- `PreToolUse` protects supported mutating/destructive paths before the action. It denies direct `workflow-state.yaml` edits, arbitrary workflow administration, unresolved decisions, stale recovery, Lane 2/3 product-code modification before execution, review/delivery edits that have not reopened implementation, missing execution approval, unavailable Git state, and execution-time user directives that have not been externalized. It still permits canonical governance artifacts under `.codex-context/`, `STRATEGY.md`, `docs/codex/specs/`, `docs/codex/plans/`, and `docs/codex/wayfinder/`. Read-only diagnosis remains available. This is a guardrail over supported tool paths, not a complete security sandbox.
...
- `PostToolUse` keeps read/search handling lightweight by marking investigation notes dirty when needed. After supported mutations, it combines the pre-mutation intent with current Git evidence, so a tool that commits its own changes cannot hide them by leaving a clean worktree. Governance refresh credit requires a successful tool result and an actual content-hash change; failed or no-op edits do not count.
...
- `SubagentStart` injects parent task identity, phase, lane, lifecycle boundaries, and a deterministic result contract: evidence/findings, risks/open gaps, and a parent next action. Fixed `Evidence`, `Risks`, and `Next action` headings are recommended for readability but are not required when the summary carries the same usable facts. `SubagentStop` gives one bounded continuation for missing lifecycle or summary evidence. It does not enforce file-level delegated scope, so parent review remains required.
