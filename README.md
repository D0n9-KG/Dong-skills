...
- A real Lane 0 direct edit uses `work-lane-0` then `mechanical-exception`. An explicit user instruction to skip brainstorming uses the one-time task-bound `spec-skipped` path; neither shortcut silently grants broader execution approval.
