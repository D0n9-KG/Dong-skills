...
Execution approval binds to a normalized plan contract: substantive `plan-progress.md` content plus any linked detailed plan. Task checkbox state, `Current Step`, and the dedicated `Checkpoints` / `存档记录` section are mutable progress metadata; changing task text, scope, runtime constraints, verification commands, or linked-plan substance requires fresh approval.
...
For a genuine Lane 0 direct edit, record the compact scope and acceptance criterion, run `workflow-state transition work-lane-0`, then `workflow-state transition mechanical-exception`. If the user explicitly says to skip brainstorming, record the compact scope, run `workflow-state decision spec-skipped`, then `workflow-state transition spec-skipped`; the task-bound skip does not approve execution.
...
Learning review is advisory. Review pending observations with `codex-learning-memory` at a meaningful milestone, before a long pause when those observations matter, or when the user explicitly asks; then save, improve, absorb, or drop them and refresh `.codex-context/learned-instincts.md`. Do not delay Stop, PreCompact, recovery, or delivery solely for pending observations.
...
When project hooks are installed and trusted, `PreToolUse` denies supported project mutations if recovery, task identity, a pending decision, the current lane/phase, execution approval, or Git state is invalid. It does not infer approval, scope, or execution authority from natural-language prompts. After an explicit user choice, run `workflow-state decision <listed-transition>` to write task/hash-bound canonical evidence, then run the matching `workflow-state transition`; do not hand-write `target_hash`. Canonical governance artifacts under `.codex-context/`, `STRATEGY.md`, `docs/codex/specs/`, `docs/codex/plans/`, and `docs/codex/wayfinder/` remain writable before execution approval; Lane 2/3 product-code edits do not. Scope, requirement, goal, acceptance, or priority changes during execution require `brainstorming-start` and fresh approval, but the assistant externalizes that change rather than a hook guessing it from wording. Bare continuation, pure status inquiry, and learning-only future preferences do not reopen scope. Dong Skills-only maintenance does not reopen business-project scope either. A successful hooked `context-recovery-eval` writes a session-scoped receipt bound to the task, handoff hash, and runtime; another session or stale receipt cannot authorize the mutation. Read-only diagnosis should remain available. These hooks are guardrails over supported tool paths, not a complete security sandbox.

`PostToolUse` leaves ordinary reads/searches debt-free and records actual project changes with invocation-scoped intent plus Git evidence, including tool-contained commits. Unclosed changes accumulate across mutations; tests, no-op commands, and commit-only operations do not erase completed refresh evidence. A no-change verification must not turn pre-existing dirty files into new freshness debt. It reminds after a verified mutation but leaves hard freshness closure to Stop/PreCompact/delivery. `SubagentStop` grades summary evidence without blocking native multi-agent completion.

Hook burden must stay proportional to verified risk. Prefer advisory output when there is no invocation-scoped evidence that the current project changed, and do not create hard debt solely from historical dirty state, a status check, or unrelated Dong Skills maintenance. Remove a hook only when its necessary safety or recovery role can be covered more simply; otherwise narrow its trigger instead of disabling the protection wholesale.
...
Write a fresh handoff at phase boundaries and before long pauses. During discussion, discovery, planning, debugging, or substantial exploration, refresh `working-notes.md` at meaningful checkpoints and before stopping or compacting; ordinary reads and searches do not create hook debt. `UserPromptSubmit` may keep a redacted advisory in `.codex-context/discussion-state.json`, but that marker does not authorize transitions or create freshness debt. `Stop`/`PreCompact` require only deterministic workflow, mutation, verification, checkpoint, recovery, and severe asset issues. The `PreCompact` hook blocks stale manual compaction. For automatic compaction, it prepends an emergency notice to `handoff-summary.md`, preserves the existing handoff below that notice, writes a raw snapshot, and allows compaction to continue, because automatic compaction may happen under context pressure where a hard block can leave the session stalled. After recovery, run `context-recovery-eval` through the installed hook path before supported mutations. Treat the emergency notice as temporary: run `asset-governance --apply` or refresh a normal handoff so the notice is archived out of the active handoff.
